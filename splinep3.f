	  PROGRAM SPLINEP3
c          PARAMETER (MMAX=24000,NMAX=217800,NBMAX=16,MNBMAX=MMAX*NBMAX, 
c     .NNBMAX=15*NMAX)
          PARAMETER (MMAX=100,NMAX=100,NBMAX=16,MNBMAX=MMAX*NBMAX, 
     .NNBMAX=15*NMAX)
     ...
     .C(NNBMAX),T(NNBMAX),JC(NNBMAX),LC(NMAX),D(NMAX), 
     .H(NMAX),R(NMAX),S(NMAX),ISRV(nMAX),NNG(NMAX),
     .U(MMAX),V(MMAX),W(MMAX),B1(NMAX),IA1(NMAX),
...
C
                    K=IPTO(3)
                    L=IAST(1)
                    CALL SPLINP3(U,V,W,A,IA,P,B,KA,JA,LA,IB,JB, 
...
1051      FORMAT(5X,'NUMBER OF DATA ',/5X,
     .'EXCEEDING THE PREVIOUS DIMENSION'///)  
          END  
C****************************     SPLINP3
          SUBROUTINE SPLINP3(X,Y,Z,A,IA,P,B,KA,JA,LA,IB,JB,
     .C,T,JC,LC,D,W,R,U,ISRV,NNG,RES,FORMA,STP,
     .MMAX,NMAX,SCX,SCY,SCU,KK,LL,TOLL,
     .IAUTO,ITABLE,IFILE,ILI,B1,IA1)
          IMPLICIT DOUBLE PRECISION(A-H,O-Z)
     ...
     .C(1),T(1),JC(1),LC(1),D(1),B1(1),IA1(1),
...
C	  A1(NV)=1.D0
	  ...
	  ENDIF
          ...
          IF(NNCC.EQ.1.AND.NNNN.LT.6) THEN
	  CALL VININTER(N1,N2,N3,N4,LA,JA,IB,NCX,NCY,NP)
	  CALL COORCLAS(NI,J,I,NIX)
	  IPNEAR=NEARPOIN(NI,NCX,X,Y,X0,Y0,STP,IB,NP)
          NV=NV+1  
C	  A1(NV)=1.D0
	  ...
	  B1(NV)=-Z(IPNEAR)*SCU
          ...
          IF(NNCC.EQ.2.AND.NNNN.LT.6) THEN
	  CALL VININTER(N1,N2,N3,N4,LA,JA,IB,NCX,NCY,NP)
	  CALL COORCLAS(NI,J,I,NIX)
	  IPNEAR=NEARPOIN(NI,NCX,X,Y,X0,Y0,STP,IB,NP)
          NV=NV+1  
C	  A1(NV)=1.D0
	  ...
	  B1(NV)=-Z(IPNEAR)*SCU
          ...
          IF(NNCC.GE.3.AND.NNNN.LT.6) THEN
	  CALL VININTER(N1,N2,N3,N4,LA,JA,IB,NCX,NCY,NP)
	  CALL COORCLAS(NI,J,I,NIX)
	  IPNEAR=NEARPOIN(NI,NCX,X,Y,X0,Y0,STP,IB,NP)
          NV=NV+1  
...
C          WRITE(6,1021) M,N,NN/N,N*((N+1)/2),NN,N*((N+1)/(2*NN))
	...
	ENDDO
     ...
     .1.D-5,100,ITE,1,0,R,U,W,N)
          ...
          ENDIF
c          CALL SZEROR(A,IA,KA,B,P,D,SZ,SZ2,MMAX,M,N,NV)
c          CALL DQVVI(A,IA,KA,T,JA,JC,LA,P,W,MMAX,M)
C  
          REWIND 1 
          IF(ILI.EQ.1) THEN
          WRITE(6,1030)
...
c          WRITE(6,1040) I,D(I),C(IS)
          WRITE(6,'(13x,i10,f15.6)') I,D(I)
130       CONTINUE 
          IF(IFILE.EQ.1) THEN
	eu=0.d0
...
140       READ(1,1010) (DATO(L),L=1,120)
          ...
          SQMV=0.D0
          ASTI=' ' 
c          IF(SZ2*P(I).GT.1.D-10) THEN  
c          SQMV=DSQRT(SZ2*P(I)) 
c          IF(DABS(B(I))/SQMV.GE.TOLL) ASTI='*' 
c          ENDIF
          SRV=1.D0-P(I)
          SQMS=0.D0
c          IF(SZ2*SRV.GT.1.D-10) SQMS=DSQRT(SZ2*SRV)
c          WRITE(6,1060) NPT,US,SGNL,SQMS,B(I),SQMV,ASTI
          WRITE(6,1061) NPT,US,SGNL,B(I)
          ENDIF
          RES(I)=B(I)
          GOTO 140 
150       NGL=M-N+NV
...
1061      FORMAT(12X,I10,F15.6,F15.6,10x,F15.6)
