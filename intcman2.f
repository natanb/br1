...
5         A=COV(2)*DEXP(DB*XCOV(2))
          DO 10 I=1,NCOV
          TCOV(I)=DA*DEXP(-DB*XCOV(I))
10        CONTINUE
          RETURN
15        A=COV(2)*DEXP(DB*XCOV(2)**2)
          DO 30 I=1,NCOV
          TCOV(I)=DA*DEXP(-DB*XCOV(I)**2)
30        CONTINUE
          RETURN
25        SRV=DEXP(-DB*XCOV(2))
          A=COV(2)/((1.D0-DC*XCOV(2)**2)*SRV)
          DO 20 I=1,NCOV
          SRV=DEXP(-DB*XCOV(I))
...
45        SRV=DEXP(-DB*XCOV(2))
          A=COV(2)/(SRV*DSIN(DC*XCOV(2))/(DC*XCOV(2)))
          TCOV(1)=DA
          DO 80 I=2,NCOV
...
70        CONTINUE
          RETURN
65        A=COV(2)/(DEXP(-DB*XCOV(2))*DCOS(DC*XCOV(2)))
          DO 50 I=1,NCOV
          TCOV(I)=DA*DEXP(-DB*XCOV(I))*DCOS(DC*XCOV(I))
50        CONTINUE
          RETURN
75        A=COV(2)/(DEXP(-DB*XCOV(2)**2)*DCOS(DC*XCOV(2)))
          DO 60 I=1,NCOV
          TCOV(I)=DA*DEXP(-DB*XCOV(I)**2)*DCOS(DC*XCOV(I))
60        CONTINUE
          RETURN
85        SRV=DEXP(-DB*XCOV(2))
          A=COV(2)/(SRV*DFJ1(DC*XCOV(2))/(DC*XCOV(2)))
          TCOV(1)=DA
          DO 120 I=2,NCOV
...
110       CONTINUE
          RETURN
105       A=COV(2)/(DEXP(-DB*XCOV(2))*DFJ0(DC*XCOV(2)))
          DO 90 I=1,NCOV
          TCOV(I)=DA*DEXP(-DB*XCOV(I))*DFJ0(DC*XCOV(I))
90        CONTINUE
          RETURN
115       A=COV(2)/(DEXP(-DB*XCOV(2)**2)*DFJ0(DC*XCOV(2)))
          DO 100 I=1,NCOV
