          PARAMETER (NMAX=1000,KMAX=1,MMAX=KMAX*NMAX,
     .NWCMAX=1000,MWCMAX=KMAX*NWCMAX,NWDMAX=1000,
...
60        CONTINUE  
     ...
     .IBANDA,JBANDA,NUMERO,INDIR,LC,JC,NMAX,NWDMAX,N,NWD,IORDI,ILI)
      print *,"Esce SPASAR legge i pesi (unita = fort.47) "
       do i=1,n
         read(47,*) p(i)
         p(i)=1./p(i)
       enddo
...
64        CONTINUE 
          ...
          IS=ICA(K,L,II)
          KK=KK+1  
          C(KK)=SNOISE(IS)*p(1)+DA(IS)  
c          C(KK)=SNOISE(IS)+DA(IS)   ver. orig
          JC(KK)=INR+K 
65        CONTINUE 
          LC(INC+L+1)=KK+1 
70        CONTINUE 
...
90        CONTINUE 
          ...
          DO 92 K=1,L  
          IS=ICA(K,L,II)
          COVA(INR+K,L)=SNOISE(IS)*p(i)+DA(IS)  
c          COVA(INR+K,L)=SNOISE(IS)+DA(IS)  ver. orig
...
122       CONTINUE 
          ...
          CALL ALGITE(C,T,JC,LC,Z,JCOVAS,JCRONA,IBANDA,JBANDA, 
     .1.D-8,30,ITE,1,1,R,U,W,M)
...
130       CONTINUE 
          ...
          JJ=J+K-L 
          IS=ICA(K,L,II)
          AA(K,L)=SNOISE(IS)*p(i)
c          AA(K,L)=SNOISE(IS) ver. orig
          AA(L,K)=AA(K,L)  
          BB(K,L)=C(JJ)
          BB(L,K)=BB(K,L)  
150       CONTINUE
