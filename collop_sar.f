          PARAMETER (NMAX=8000,KMAX=1,MMAX=KMAX*NMAX,
     .NWCMAX=8000,MWCMAX=KMAX*NWCMAX,NWDMAX=8000,
...
*     .MMWMAX=MMAX*MWCMAX,MMW2MX=MMWMAX/2,LLWMAX=MAX(NNWMAX,MMW2MX))
          ...
          CHARACTER*120 CDATO
          DIMENSION V(MMAX),Z(MMAX),LC(MMAX1),
     .ICOVA(MWCMAX,3),COVA(MWCMAX,3),KONT(NMAX1),
     .R(MMAX),U(MMAX),W(MMAX),
     .X(NMAX),Y(NMAX),IPX(NMAX),IPY(NMAX),LPX(NMAX),LPY(NMAX), 
     .JCRONA(MMAX1),IBANDA(NMAX),JBANDA(NMAX),  
     ...
     .TGL(6),FORMA(20),TITOLO(20),ND(7,3),EC(3)
          DIMENSION JC(LLWMAX),JCOVAS(LLWMAX)
          DIMENSION C(MMW2MX),T(MMW2MX)
...
C  
          ...
          WRITE(6,1020) TITOLO 
          READ(5,*) ICASO,ITIPO
          READ(5,*) ISCRI,ITABLE,IFILE,IMATRI,IQUO,IORDI,TOLL  
          READ(5,*) SCX,SCY,SCU,SCV,SCW
          READ(5,1010) FORMA
...
5         CONTINUE 
          J=0  
10        J=J+1
          READ(1,1000,END=30) (DATO(I),I=1,120)
          K=0
          DO 15 I=1,120
...
50        READ(1,1000)(DATO(I),I=1,120)
          IF(ITIPO.EQ.1) READ(CDATO,FORMA) NPT,XS,YS,US
          IF(ITIPO.EQ.2) READ(CDATO,FORMA) NPT,XS,YS,US,VS
          IF(ITIPO.EQ.3) READ(CDATO,FORMA) NPT,XS,YS,US,VS,WS 
          IF(NPT.EQ.0) GOTO 55
          IP1=0
          IP2=0
          IP3=0
...
58        CONTINUE
          ...
          EC(I)=EC(I)/LP(I)
59        CONTINUE
          IF(ITIPO.EQ.1) WRITE(CDATO,FORMA) N,EX/SCX,EY/SCY,E(1)/SCU 
          IF(ITIPO.EQ.2) WRITE(CDATO,FORMA) N,EX/SCX,EY/SCY,E(1)/SCU
     .,E(2)/SCV
          IF(ITIPO.EQ.3) WRITE(CDATO,FORMA) N,EX/SCX,EY/SCY,E(1)/SCU
     .,E(2)/SCV,E(3)/SCW
...
60        CONTINUE  
	print *,"ENTRA SPASAR"
C  
        CALL SPASAR(XCOVT,X,Y,IPX,IPY,LPX,LPY,IWD,IPUNT,JCRONA,JCOVAS,
...
75        CONTINUE 
          ...
          DO 80 L=1,IC 
          DO 80 K=1,IR
c IF(DELTA .GT. 0.D0) THEN
          COVA(INR+K,L)=GASSAR(DELTA,XCOVT,JJ,II,K,L,
...
C     
     ...
     .1.D-3,5,ITE,1,1,R,U,W,M)
          ...
          WRITE(10,*) N
          WRITE(10,1093) (X(I),Y(I),I=1,N) 
          WRITE(10,1095) (IPX(I),IPY(I),LPX(I),LPY(I),NTIPO(I),
     .INDIR(I),I=1,N)  
          ENDIF
C     
          REWIND 1
...
200       CONTINUE 
          ENDIF
210       CONTINUE
	write(32,'(i8,2f10.3,f10.5,f10.5,f8.3,f10.5,f8.3)') 
     .NPT,X(I),Y(I),V(I)+EC(1),D(1)+EC(1),AA(1,1),E(1),BB(1,1)
...
1191      FORMAT(5X,'WRONG CASE AND/OR TYPE OF DATA'///)
          END  
          SUBROUTINE SPASAR(XCOVT,X,Y,IPX,IPY,LPX,LPY,IWD,IPUNT,
     .JCRONA,JCOVAS,IBANDA,JBANDA,NUMERO,INDIR,LC,JC,  
     .NMAX,NWDMAX,N,NWD,IORDI,ILI) 
          IMPLICIT DOUBLE PRECISION(A-H,O-Z)
          DIMENSION X(*),Y(*),IPX(*),IPY(*),LPX(*),LPY(*),IWD(*),  
     .IPUNT(*),JCRONA(*),JCOVAS(*),IBANDA(*),JBANDA(*),
     .NUMERO(*),INDIR(*),LC(*),JC(*)
