	  PROGRAM SPLINEP2
          PARAMETER (MMAX=3000,NMAX=3000,NBMAX=16,MNBMAX=MMAX*NBMAX, 
     .NNBMAX=25*NMAX)
...
C
                    K=IPTO(3)
                    L=IAST(1)
                    CALL SPLINP2(U,V,W,A,IA,P,B,KA,JA,LA,IB,JB, 
...
1051      FORMAT(5X,'NUMBER OF DATA ',/5X,
     .'EXCEEDING THE PREVIOUS DIMENSION'///)  
          END  
C*****************************     SPLINP2
          SUBROUTINE SPLINP2(X,Y,Z,A,IA,P,B,KA,JA,LA,IB,JB,
...
50        CONTINUE 
	  ...
	  DO I =2,NCY-1
	   DO J =2,NCX-1
	    ...
	    YC=Y0+STP/2+(I-1)*STP
  	    NC=(I-1)*NCX+J
	    IF(NNG(NC).LE.1) WRITE(11,*) NC,NNG(NC),XC,YC
	   ENDDO
          ENDDO
...
c          B(M)=-(Z(IPNEAR)-EU)*SCU  
	b(m)=-z(ipnear)*scu
	  write(*,'(2i5,3f15.6)')i,ipnear,z(ipnear),x(ipnear),y(ipnear)
100       CONTINUE 
...
110       CONTINUE 
	  ...
	  P(M)=100000.D0
c        B(M)=-(Z(IPNEAR)-EU)*SCU  
	  write(*,'(3i5,3f15.6)')i,j,ipnear,z(ipnear),x(ipnear),y(ipnear)
   	  B(M)=-Z(IPNEAR)*SCU
	  ENDIF

c          IF(NNCC.EQ.1.AND.NNNN.LT.2) THEN
...
c          IF(NNCC.EQ.2.AND.NNNN.LT.3) THEN
c	  CALL VININTER(N1,N2,N3,N4,LA,JA,IB,NCX,NCY,NP)
c	  CALL COORCLAS(NI,J,I,NIX)
c	  IPNEAR=NEARPOIN(NI,NCX,X,Y,X0,Y0,STP,IB,NP)
c         NV=NV+1  
...
c          ENDIF
	  ...
	  IF(NNNN.EQ.4.AND.NNCC.LT.4) THEN
	  CALL VININTER(N1,N2,N3,N4,LA,JA,IB,NCX,NCY,NP)
	  CALL COORCLAS(NI,J,I,NIX)
	  IPNEAR=NEARPOIN(NI,NCX,X,Y,X0,Y0,STP,IB,NP)
	  NV=NV+1  
	  ...
	  P(M)=100000.D0
   	  B(M)=-Z(IPNEAR)*SCU
          ...
          IF(NNNN.GT.0.AND.NNNN.LT.4) THEN
	  CALL VININTER(N1,N2,N3,N4,LA,JA,IB,NCX,NCY,NP)
	  CALL COORCLAS(NI,J,I,NIX)
	  IPNEAR=NEARPOIN(NI,NCX,X,Y,X0,Y0,STP,IB,NP)
          NV=NV+1  
	  ...
	  KA(M)=1
	  P(M)=100000.D0
	  write(*,'(3i5,3f15.6,i5)')i,j,ipnear,z(ipnear),x(ipnear)
     .    ,y(ipnear),nnnn
	  B(M)=-Z(IPNEAR)*SCU
          ENDIF
...
140       READ(1,1010) (DATO(L),L=1,120)
          ...
          IF(ASTI.EQ.'*') THEN
           WRITE(22,'(I5,5X,2(F15.4,10X),F15.5,5X,A1,F15.4)') 
     +     NPT,X,Y,US,ASTI,B(I)
          ELSE
           WRITE(22,'(I5,5X,2(F15.4,10X),F15.5)') NPT,X,Y,US
          ENDIF
          ENDIF
          RES(I)=B(I)
          GOTO 140 
