          PARAMETER (MMAX=10000,NMAX=5000,NBMAX=4) 
          ...
          DIMENSION A(MMAX,NBMAX),IA(MMAX,NBMAX),P(MMAX),B(MMAX),  
     .KA(MMAX),C(NMAX,NBMAX),D(NMAX),H(NMAX),NNG(NMAX),
     .U(MMAX),V(MMAX),W(MMAX),  
     .DATO(120),PCHAR(120),FORMA(20),FORMA2(20),FORMA3(20),TITOLO(20), 
     .IPTO(6),IAST(3),RESI(15),RES2(15),RES3(15)
...
C           
                    K=IPTO(2)
                    L=IAST(1)
                    CALL SPLINT(A,IA,P,B,KA,C,D,H,NNG,U,FORMA,STP, 
     .              MMAX,NMAX,NBMAX,SCZ,SCU,K,L,TOLL,IAUTO,IFILE,ILI)  
          IF(ITIPO.GE.2) THEN  
                    K=IPTO(3)
                    L=IAST(2)
                    CALL SPLINT(A,IA,P,B,KA,C,D,H,NNG,V,FORMA2,STP2,
     .              MMAX,NMAX,NBMAX,SCZ,SCV,K,L,TOLL,IAUTO,IFILE,ILI)  
          ENDIF
          IF(ITIPO.EQ.3) THEN  
                    K=IPTO(4)
                    L=IAST(3)
                    CALL SPLINT(A,IA,P,B,KA,C,D,H,NNG,W,FORMA3,STP3,
     .              MMAX,NMAX,NBMAX,SCZ,SCW,K,L,TOLL,IAUTO,IFILE,ILI)  
          ENDIF
C
          IF(IFILE.EQ.1) THEN
          REWIND 1
