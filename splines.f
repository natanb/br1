          PARAMETER (MMAX=1000,NMAX=800,NBMAX=64,MNBMAX=MMAX*NBMAX, 
     .NNBMAX=172*NMAX)
...
C
                    K=IPTO(4)
                    L=IAST(1)
                    CALL SPLINS(A,IA,P,B,KA,JA,LA,IB,JB, 
     .              C,T,JC,LC,D,H,R,S,ISRV,NNG,U,FORMA,STP,MMAX,NMAX,
     .              SCX,SCY,SCZ,SCU,K,L,TOLL,IAUTO,ITABLE,IFILE,ILI) 
          IF(ITIPO.GE.2) THEN  
                    K=IPTO(5)
                    L=IAST(2)
                    CALL SPLINS(A,IA,P,B,KA,JA,LA,IB,JB, 
     .              C,T,JC,LC,D,H,R,S,ISRV,NNG,V,FORMA2,STP2,MMAX,NMAX,  
     .              SCX,SCY,SCZ,SCV,K,L,TOLL,IAUTO,ITABLE,IFILE,ILI) 
          ENDIF
          IF(ITIPO.EQ.3) THEN  
                    K=IPTO(6)
                    L=IAST(3)
                    CALL SPLINS(A,IA,P,B,KA,JA,LA,IB,JB, 
     .              C,T,JC,LC,D,H,R,S,ISRV,NNG,W,FORMA3,STP3,MMAX,NMAX,  
     .              SCX,SCY,SCZ,SCW,K,L,TOLL,IAUTO,ITABLE,IFILE,ILI) 
          ENDIF
C
          IF(IFILE.EQ.1) THEN
          REWIND 1
